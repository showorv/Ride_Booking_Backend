...
const createDriver = async (payload: iDriver, decodedToken: JwtPayload) => {
  ...
  if (!vehicleNumber && !license) {
    throw new AppError(
      401,
      "cannot request without valid vehicle number and license documents"
    );
  }
...
const suspenseDriver = async (driverId: string, payload: Partial<iDriver>) => {
  ...
  const updateDriver = await Driver.findByIdAndUpdate(
    driverId,
    { isSuspend: true, isApproved: false },
    { new: true, runValidators: true }
  );
...
const acceptedRide = async (rideId: string, decodedToken: JwtPayload) => {
  ...
  if (!driver || !driver.isApproved) {
    throw new AppError(
      401,
      "cannot accept ride you're not a driver or not approved yet"
    );
  }
  ...
  if (driver.onlineStatus === activeStatus.OFFLINE) {
    throw new AppError(
      401,
      "you're in offline mode. you've to online to accept ride"
    );
  }
...
const cancledRideByDriver = async (
  rideId: string,
...
const setOnlineStatus = async (
  ...
  const updateOnlineStatus = await Driver.findByIdAndUpdate(
    driver._id,
  ...
  if (payload.status === rideStatus.COMPLETED) {
    await Driver.findByIdAndUpdate(
      driver._id,
      {
        isAvailable: true,
        currentRide: null,
        totalEarning: (driver.totalEarning as number) + ride.fare,
      },
      { runValidators: true }
    );
  }
...
const getAvaiblableRidesForDriver = async (decodedToken: JwtPayload) => {
  ...
  const availableRequest = await Ride.find({ status: rideStatus.REQUESTED })
    .select("pickupLocation dropLocation isCancelledByRider createdAt")
    .sort({ createdAt: -1 });
...
const viewEarnignHistory = async (decodedToken: JwtPayload) => {
  ...
  const earningHistory = await Ride.find({
    ...
    .select("fare timeStamps.completedAt")
